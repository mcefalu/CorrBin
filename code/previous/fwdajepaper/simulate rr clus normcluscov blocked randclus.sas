****************************************************************************************************************;
*																						 	 				   *;
* Macro to Simulate Clustered Data with Blocked Randomisation of Clusters and a Normal Cluster Level Covariate *;
*																						 	 				   *;
****************************************************************************************************************;
...
/* data = name of dataset
   ...
   4) randomly allocates clusters a covariate value from N(meannc,varnc) distribution
   5) resamples from covariate distribution until probability < 1 if necessary
   ...
   note: ignore pbc, rhobc, rrbc, rhonc (these are used to simulate more complicated datasets) */
	  	...
	  	do simulation = 1 to &nsim.;
	   		do cluster_id = 1 to &nclus.;
				...
				do until (prob < 1);
					...
					normcov = &meannc. + sqrt(&varnc.)*rannor(i*cluster_id*simulation);	
...
%simulate(test, 1000, 50, 0, 10, 0.1, 2, 0.1, , , , 0.5, 0.25, , 2);
...
proc univariate data=temp;
	var normcov random cluster_size; 		
	histogram normcov random cluster_size;
run;
...
%simulate(test, 1000, 50, 0, 10, 0.1, 2, 0.2, , , , 0.5, 0.25, , 2);
